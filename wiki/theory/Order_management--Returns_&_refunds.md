---
author: Walmart 官方文档
auditor: 知识库管理员
status: verified
audit_date: 2026-04-26
tags: [官方文档, 知识库]
---

# 退款与退货（Returns & Refunds）

> 退货政策、退款处理、Keep It 规则、Item-Level 退货规则完整指南。

## 核心操作

| 操作 | 说明 |
|------|------|
| [[theory/returns-policy]] | 退货政策（退货期、退货运费政策、RSS）|
| [[theory/Issue-a-standard-refund-in-Seller-Center]] | 标准退款处理 |
| [[theory/Issue-adjustments-or-non-standard-refunds-in-Seller-Center]] | 非标准退款/超额退款 |
| [[theory/Add-a-Keep-It-Rule]] | Keep It 规则（客户保留商品获得退款）|
| [[theory/Create-item-level-return-rules]] | 商品级退货规则（批量模板）|
| [[theory/add-replacement-rules]] | Replacement 替换规则 |
| [[theory/Request-a-return-policy-exemption]] | 退货政策豁免申请 |
| [[theory/returns-insights-overview]] | 退货洞察报告 |

## Keep It 规则

**全额 Keep It：**
- 设置部门/商品价格上限/退单总额上限
- 客户保留商品，全额退款

**Partial Keep It：**
- 设置退货原因对应的退款百分比
- 客户保留商品，获得部分退款

## 关键规则

- 退货期至少 **30 天**（从发货日期后 7 天开始计算）
- 退货费用可由卖家/买家承担或免费
- Item-Level 退货规则优先于类别级规则
- 已被豁免的商品无法应用 Partial Keep It Rules

## 关联页面

- [[theory/Order_management]] — 订单管理总览
- [[theory/wfs-returns-overview]] — WFS 退货处理
- [[theory/Returns-Shipping-Service-RSS-policy]] — RSS 退货运输政策

---

> 源文件目录：`raw/Order_management--Returns_&_refunds/`